...
4. **Write per post** (one transaction per post, so one bad row can't poison the batch):
   - `ok: true` → INSERT snapshot; UPDATE post `latest_* = metrics`, `latest_snapshot_at = capturedAt`; if the creator is a placeholder (`handle` starts with `placeholder:`) and `authorHandle` resolved, merge deterministically IN THE SAME TRANSACTION: if a creator with `(platform, authorHandle)` already exists, re-point this post's `creator_id` to it and delete the placeholder if it has no remaining posts; otherwise rename the placeholder to the real handle (respecting the UNIQUE constraint — on conflict, the re-point branch applies). The rename branch also fills `display_name`, `avatar_url`, and `profile_url` (derived from platform + resolved handle) from the provider metadata — a resolved creator must not keep its stand-in post-URL `profile_url`.
...
5. **Respond** `{ selected, refreshed, removed, skipped, errors, durationMs, degraded }` (`skipped` = posts on platforms with no configured provider per spec 003's registry-null rule; `degraded` = the >50%-errors flag from Guards) — also `console.info` one structured summary line for Vercel logs.
