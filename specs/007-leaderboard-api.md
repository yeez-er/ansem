...
Public tRPC read surface: ranked boards (weekly / all-time, per-platform or combined), creator profiles, and the recent-posts feed that the UI renders.
...
### `leaderboard.get`
- Input: `{ period: z.enum(['weekly','alltime']), platform: z.enum(['x','tiktok','instagram','all']).default('all'), cursor: z.number().int().min(0).default(0), limit: z.number().int().min(1).max(100).default(25) }`
- Weekly: per spec 006 semantics — baseline = newest snapshot before `weekWindow(now).start`, latest = newest within window, per-post delta score, summed per creator. Implemented as one SQL query (`DISTINCT ON` / lateral joins), NOT a per-creator N+1 loop; **bound the scan**: consider only posts with `latest_snapshot_at >= window.start − 7d`.
...
- Returns `{ creator: PublicCreator, alltime: ScoreSummary, weekly: ScoreSummary, posts: PublicPost[] }` — posts capped at 50, newest first, `PublicPost = { id, url, caption, postedAt, views, likes, comments, shares, score }` (strings for counts). Unknown id or banned creator → **`null`** (not `{}`, not a throw).
...
- [ ] Weekly board counts ONLY within-window deltas: a post with 1M views all captured before the window contributes 0 this week
